...
def extract_pi_parts(pi_string):
...
def handle_special_cases(pi_transcription, default):
    if pi_transcription in ['*', '**']:
        return default
    return pi_transcription if pi_transcription else None


def process_corpus_line(line):
    sf = None
    ...
    if len(parts) > 1:
        pi_transcriptions_parts = extract_pi_parts(parts[1].strip())
    else:
        pi_transcriptions_parts = [se_word, None,
                                   None, None]  # Default to SE word

    pi_entry = {
        "L1": handle_special_cases(pi_transcriptions_parts[0], se_word),
        "L2": handle_special_cases(pi_transcriptions_parts[1], pi_transcriptions_parts[0] or se_word),
        "L3": handle_special_cases(pi_transcriptions_parts[2], pi_transcriptions_parts[0] or se_word),
        "FM": handle_special_cases(pi_transcriptions_parts[3], pi_transcriptions_parts[1] or pi_transcriptions_parts[0] or se_word)
    }

    return se_word, {"whole": whole_line, "PI": pi_entry, "Sf": sf}


def process_corpus_data(lines):
    word_entries = {}
    skip_section = False
    for line in lines:
        if "::::::::::" in line.strip():
        ...
        se_word, word_entry = process_corpus_line(line)
...
def main():
    ...
    corpus_files = ['corpus_1000.txt', 'corpus_misc.txt', 'corpus_auto.txt']
    pi_dictionary = {}

    for file_name in corpus_files:
        file_path = os.path.join(corpus_folder, file_name)
        lines = read_corpus_file(file_path)
        if lines:
            word_entries = process_corpus_data(lines)
    ...
    try:
        ...
        print("Word entries successfully written to 'dict/pi_dictionary.json'")
    except IOError as e:
