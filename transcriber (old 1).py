...
class Transcriber:
    def __init__(self):
        ...
        output_text = self.perform_preliminar_replacements(input_text)
        ...
        output_text = self.update_words_for_piss_variation(
            output_text, pi_dictionary, chosen_variation)
        output_text = self.transform_words_with_s_suffix(
            output_text, pi_dictionary, chosen_variation)
    ...
    def transcribe(self, input_text, variation):
        # Main method to transcribe the input text
        # This might involve calling the above methods in a specific sequence
        # and handling the overall text transformation process
        return
